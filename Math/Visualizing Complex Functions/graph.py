    ...
    def reset_display(self, grid=True):
        ...
        if grid:
            ...
            for i in range(self.x_scale[0], self.x_scale[1]):
                ...
                cv2.line(self.input, (x_pos, 0), (x_pos, self.Y), colors['dark blue'], 1)
            #   Horizontal
            for i in range(self.y_scale[0], self.y_scale[1]):
                y_pos = i*self.Y//(self.y_scale[1] - self.y_scale[0]) + self.Y//2
                cv2.line(self.input, (0, y_pos), (self.X, y_pos), colors['dark blue'], 1)
        # Axes
        cv2.line(self.input, (self.X//2, 0), (self.X//2, self.Y), colors['light grey'], 3)
        cv2.line(self.input, (0, self.Y//2), (self.X, self.Y//2), colors['light grey'], 3)
        ...
        if grid:
            ...
            for i in range(self.x_scale[0], self.x_scale[1]):
                x_pos = i*self.X//(self.x_scale[1] - self.x_scale[0]) + self.X//2
                cv2.line(self.output, (x_pos, 0), (x_pos, self.Y), colors['grey'], 1)
            #   Horizontal
            for i in range(self.y_scale[0], self.y_scale[1]):
                y_pos = i*self.Y//(self.y_scale[1] - self.y_scale[0]) + self.Y//2
                cv2.line(self.output, (0, y_pos), (self.X, y_pos), colors['grey'], 1)
            ...
            for i in range(self.x_scale[0], self.x_scale[1]):
                ...
                cv2.polylines(self.output, np.int32([points]), True, colors['dark blue'])
            #   Horizontal
            for i in range(self.y_scale[0], self.y_scale[1]):
        ...
        # Axes
        cv2.line(self.output, (self.X//2, 0), (self.X//2, self.Y), colors['light grey'], 3)
        cv2.line(self.output, (0, self.Y//2), (self.X, self.Y//2), colors['light grey'], 3)
    ...
    def draw_input(self, event, x, y, flags, param):
        if event == cv2.EVENT_LBUTTONDOWN:
            ...
            if self.drawing:
                # Drawing input plane
                if self.b < x < self.X - self.b:
                    cv2.circle(self.input, (x,y), 2, colors[color_names[self.c]], -1)

                # Drawing output plane
                u, v = self.complex_to_cv( self.f(self.cv_to_complex(x, y)) )
                if self.b < u < self.X - self.b:
                    cv2.circle(self.output, (u,v), 2, colors[color_names[self.c]], -1)

        elif event == cv2.EVENT_LBUTTONUP:
            self.drawing = False
            self.c += 1
            self.c %= len(color_names)
